...
LR_PRETRAIN = 0.000008
LR_FINETUNE = 0.00008
WARMUP_STEPS = 500


class CrossLingualModel(nn.Module):
    def __init__(self, num_lg=2, num_labels=len(global_label_list)):
        super().__init__()
    ...
    def forward(self, input_ids, attention_mask, ner_labels=None, lg_in=0):
        ...
        hidden_state = self.lg_projections[self.key_to_index["{}-{}".format(lg_in, self.lg_out)]](sequence_output)

        att_score = nn.Softmax(dim=-1)(
            torch.matmul(hidden_state, self.align_embedding.T) / self.hidden_size ** 0.5)  # T * V
        att_score = self.dropout(att_score)

        att_output = torch.matmul(att_score, self.align_embedding)

        att_output = self.dropout(att_output)
        logits = self.ner_classifier(att_output)
...
def main():
    ...
    checkpoint = torch.load("ckpt-1", map_location='cpu')
...
def _evaluate(model, dataloader, lg):
    ...
    for batch in tqdm(dataloader):
        ...
        _, logits = model(batch["input_ids"], batch["attention_mask"], lg_in=1 if lg == "de" else 0)
        logits = logits.cpu()
