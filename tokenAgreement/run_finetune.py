...
LANGUAGE_IDS = [args.lg]  # , "es", "nl", "pl", "ru", "it", "fr", "pt"]


# https://github.com/Babelscape/wikineural
def process_task_data():
    raw_datasets = load_dataset("wikiann", LANGUAGE_IDS[0],
    ...
    for lg in LANGUAGE_IDS:
        eval_dataloaders[lg] = DataLoader(processed_raw_datasets["test"], collate_fn=default_data_collator,
                                          batch_size=64)
...
def get_train_dataloader():
    raw_datasets = load_dataset("wikiann", "en", keep_in_memory=True,
                                cache_dir="/brtx/605-nvme1/yukunfeng/cross-lingual/tokenAgreement/data/huggingface")  # ["train_en", "val_en"])

    train_en_tokens = raw_datasets["train"]["tokens"][:]
    train_en_labels = raw_datasets["train"]["ner_tags"][:]
