...
res_path = "res/paws-{}-{}".format(args.lg, args.ratio)
...
def process_task_data():
    eval_dataset = load_dataset("xtreme", "PAWS-X." + LANGUAGE_IDS[0], split="test")  # ["train_en", "val_en"])

    _labels = eval_dataset["label"][:]
    labels = [int(l) for l in _labels]
    eval_dataset = eval_dataset.remove_columns(["label"])
    eval_dataset = eval_dataset.add_column("label", labels)
...
def get_train_dataloader():
    train_dataset = load_dataset("xtreme", "PAWS-X.en", split="train")

    train_premise = train_dataset["sentence1"][:]
    train_hypothesis = train_dataset["sentence2"][:]

    _labels = train_dataset["label"][:]
    labels = [int(l) for l in _labels]
    ...
    train_dataset = train_dataset.add_column("sentence1", new_premise)
    train_dataset = train_dataset.add_column("sentence2", new_hypothesis)
